...
func newProxyServer(
	...
	klog.InfoS("Detected node IP", "nodeIP", nodeIP.String())
	...
	if dualStack {
		...
		if primaryProtocol == utiliptables.ProtocolIPv4 {
			...
			if _, err := ipt[1].ChainExists(utiliptables.TableNAT, utiliptables.ChainPostrouting); err != nil {
				klog.ErrorS(err, "No iptables support for IPv6")
				dualStack = false
			}
		} else {
	...
	if dualStack {
		klog.V(0).InfoS("kube-proxy running in dual-stack mode", "IP family", iptInterface.Protocol())
	} else {
		klog.V(0).InfoS("kube-proxy running in single-stack mode", "IP family", iptInterface.Protocol())
	}
